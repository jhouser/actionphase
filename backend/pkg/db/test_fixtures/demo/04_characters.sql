...
BEGIN
  ...
  --
  -- Only bio is is_public=true. Skills/inventory/numbers are false to match
  -- CharacterSheet.saveJsonField, which hardcodes false for all three: those
  -- tabs are gated per viewer, not per row.
