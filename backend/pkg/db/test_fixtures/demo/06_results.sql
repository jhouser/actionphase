...
--
-- released_at: MUST mirror sent_at. Since staged reveals shipped, the recipient
-- read path (GetUserResults) blanks the content of any row with released_at
-- NULL, so a published fixture row without it arrives at the player as an empty
-- string — the result renders blank with no error anywhere. CreateActionResult
-- sets both columns together, so fixtures must too.
--
-- The ONLY rows that legitimately have released_at NULL while published are
-- staged parts awaiting their timer; those also carry parent_result_id and
-- reveal_delay_minutes (see 06b_staged_results.sql). A row with a NULL
-- released_at and no parent is not a state the application can produce.
