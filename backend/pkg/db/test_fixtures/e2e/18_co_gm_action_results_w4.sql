-- E2E Test Fixture for Co-GM Action Results (Worker 4)
-- Game ID: 349 (worker 4 offset: 40000 → game 349)
...
DECLARE
  ...
  worker_game_id_offset INTEGER := 40000;
BEGIN
  game_id := 349 + worker_game_id_offset;

  DELETE FROM games WHERE id = game_id;

  gm_id        := get_worker_user_id('TestGM', 4);
  audience2_id := get_worker_user_id('TestAudience2', 4);
  player1_id   := get_worker_user_id('TestPlayer1', 4);
...
SELECT 'E2E Co-GM Action Results fixture (worker 4) created successfully!' AS message;
