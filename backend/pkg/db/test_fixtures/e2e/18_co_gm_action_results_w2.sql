-- E2E Test Fixture for Co-GM Action Results (Worker 2)
-- Game ID: 349 (worker 2 offset: 20000 → game 349)
...
DECLARE
  ...
  worker_game_id_offset INTEGER := 20000;
BEGIN
  game_id := 349 + worker_game_id_offset;

  DELETE FROM games WHERE id = game_id;

  gm_id        := get_worker_user_id('TestGM', 2);
  audience2_id := get_worker_user_id('TestAudience2', 2);
  player1_id   := get_worker_user_id('TestPlayer1', 2);
...
SELECT 'E2E Co-GM Action Results fixture (worker 2) created successfully!' AS message;
