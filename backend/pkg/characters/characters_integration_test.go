...
func TestCharacterAPI_Authorization(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name:           "character owner cannot edit numbers (stats)",
			endpoint:       fmt.Sprintf("/api/v1/characters/%d/data", character.ID),
			method:         "POST",
			token:          playerToken,
			expectedStatus: http.StatusForbidden,
			...
			reason: "only GMs can edit character numbers",
		},
		{
			name:           "GM can edit character skills (stats)",
			endpoint:       fmt.Sprintf("/api/v1/characters/%d/data", character.ID),
			method:         "POST",
			token:          gmToken,
			expectedStatus: http.StatusNoContent,
			...
			reason: "GMs should be able to edit character skills",
		},
		{
