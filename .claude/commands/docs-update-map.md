...
| `backend/pkg/http/root.go` | `backend/pkg/docs/openapi.yaml`, `.claude/reference/API_DOCUMENTATION.md`, `docs-site/developer/architecture/adrs/004-api-design-principles.md` | Added/removed/renamed routes; changed auth middleware on a route. **Run `just check-api-docs`** — the spec drifts silently because Pass 1's greps don't match YAML path keys. The spec's base is `/api/v1`, so a root-mounted route (e.g. `/ping`) is unreachable as documented |
| `backend/pkg/docs/openapi.yaml` | `backend/pkg/http/root.go` (the reverse check) | A spec entry whose handler was removed — it lingers as a phantom path and returns 401 (auth middleware) rather than 404, hiding the cause |
