...
| `backend/pkg/http/root.go`, any `huma_api.go` | `backend/pkg/docs/openapi.gen.yaml`, `.claude/reference/API_DOCUMENTATION.md`, `docs-site/developer/architecture/adrs/004-api-design-principles.md` | Added/removed/renamed routes; changed auth middleware on a route. **Run `just gen-openapi`** and commit the regenerated spec — `just check-api-docs` fails when it is stale |
| `backend/pkg/docs/spec_metadata.go` | `backend/pkg/docs/openapi.gen.yaml` | Document metadata (info, servers, tags) and the `/ping` operation live here now, not in a YAML file. A new tag must be added to `specTags()` or `TestSpecTagsCoverEveryOperation` fails |
